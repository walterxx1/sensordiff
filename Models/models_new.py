...
class CrossAttention(nn.Module):
    def __init__(self, dim, context_dim=None, num_heads=8, dim_head=64, dropout=0.0):
        ...
        self.scale = dim_head ** -0.5  # Scaling factor to prevent large attention scores

        # If context_dim is not defined, set it to the same as input dim
        context_dim = context_dim if context_dim is not None else dim

        # Linear transformations for Query, Key, Value
        self.to_q = nn.Linear(dim, self.inner_dim, bias=True)
        self.to_kv = nn.Linear(context_dim, self.inner_dim * 2, bias=True)  # Output for both Key and Value
    ...
    def forward(self, x, context, mask=None):
        ...
        k, v = self.to_kv(context).chunk(2, dim=-1)  # Split into Key and Value
        ...
        attn = torch.einsum('bhqd, bhkd -> bhqk', q, k) * self.scale  # [batch_size, num_heads, seq_len, context_len]

        # Apply mask if provided
        if mask is not None:
            mask = mask[:, None, None, :]  # Broadcast mask to match attention shape
            attn = attn.masked_fill(mask == 0, float('-inf'))

        # Softmax over the context length dimension
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # Weighted sum of values
        out = torch.einsum('bhqk, bhvd -> bhqd', attn, v)  # [batch_size, num_heads, seq_len, dim_head]
        out = out.transpose(1, 2).reshape(b, n, self.inner_dim)  # Reshape back to [batch_size, seq_len, num_heads * dim_head]
